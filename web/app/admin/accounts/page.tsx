...
import type { AccountRow, AdminBalance, AdminRefundable, AdminTxn } from "@/lib/types";
...
export default function AccountsPage() {
  const state = useAsync<AccountRow[]>(() => api.get<AccountRow[]>("/v1/admin/accounts"));
          ...
          <div className="panel">
            <h2>
              ...
              <span className="muted" style={{ fontWeight: 400 }}>
                行を選択して残高を確認・編集(System Code・IDm・IDi の三つ組で識別)
              </span>
            </h2>
            <div className="row" style={{ marginBottom: 12 }}>
              <div className="field">
                <label>System Code</label>
                <input className="mono" placeholder="0x0003" style={{ width: 110 }} value={sc} onChange={(e) => setSc(e.target.value)} />
              </div>
              <div className="field">
                <label>IDm(16桁hex)</label>
                <input className="mono" placeholder="0102030405060708" value={idm} onChange={(e) => setIdm(e.target.value)} />
              </div>
              <div className="field">
                  ...
                  {accounts.length === 0 ? (
                    ...
                    accounts.map((a) => (
                      <tr
                        ...
                        className="clickable"
                        onClick={() => setSelected({ sc: String(a.system_code), idm: a.idm, idi: a.idi })}
                      >
...
function AccountDetail({ acct, onAccountsChanged }: { acct: AcctKey; onAccountsChanged: () => Promise<void> }) {
  const toast = useToast();
  const path = `/v1/admin/accounts/${encodeURIComponent(acct.sc)}/${encodeURIComponent(acct.idm)}/${encodeURIComponent(acct.idi)}`;
  const filter = qs({ limit: 20, system_code: acct.sc, idm: acct.idm, idi: acct.idi });
  ...
  const refund = async (paymentId: string, refundable: number) => {
    ...
    try {
      const r = await api.post<{ amount: number }>("/v1/admin/refunds", { payment_id: paymentId, amount });
  ...
  const voidPayment = async (paymentId: string) => {
    ...
    try {
      const r = await api.post<{ amount: number }>(`/v1/admin/payments/${encodeURIComponent(paymentId)}/void`);
    ...
    <Async state={state}>
      {({ bal, txns, refundable }) => (
        <div className="panel">
          ...
          <h3 style={{ marginBottom: 8 }}>残高の調整(台帳に adjustment として記帳)</h3>
          <div className="row" style={{ marginBottom: 6 }}>
            ...
            <div className="field">
              <label>金額(円)</label>
              <input type="number" min={1} value={adjAmount} onChange={(e) => setAdjAmount(e.target.value)} placeholder="1000" />
            </div>
          ...
          <p className="muted" style={{ margin: "0 0 18px" }}>
            入金は新しい 6ヶ月バケットを作成、引落は期限が近い順に消費(残高不足は不可)。
          </p>
                  ...
                  refundable.map((p) => (
                    <tr key={p.id}>
                      ...
                      <td style={{ textAlign: "right", whiteSpace: "nowrap" }}>
                        <button className="sm" onClick={() => refund(p.id, p.refundable)}>
                          返金
                        </button>
                        <button className="sm" onClick={() => voidPayment(p.id)}>
                          取消
                        </button>
                      </td>
                    </tr>
