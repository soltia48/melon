...
export default function MerchantUsersPage() {
  ...
  const createStaff = async () => {
    ...
    try {
      await api.post("/v1/users", { name: name.trim(), email: email.trim(), password });
      toast("スタッフを追加しました");
      setName("");
      setEmail("");
      setPassword("");
  ...
  const changePassword = async () => {
    if (!curPw || !newPw) return toast("現在と新しいパスワードを入力してください");
    try {
      await api.post("/v1/auth/password", { current_password: curPw, new_password: newPw });
  ...
  return (
    <>
      <div className="panel">
        <h2>スタッフを追加</h2>
        <p className="muted">自店のユーザーのみ作成できます(パスワードは 10 文字以上)。</p>
        <div className="row">
          <div className="field">
            <label>名前</label>
                        ...
                        <tr key={u.id}>
                          ...
                          <td style={{ textAlign: "right" }}>
                            {self ? null : (
                              <button
                                className="sm"
                                onClick={() => setStaffStatus(u.id, u.status === "active" ? "disabled" : "active")}
                              >
