...
利用者自身が、自分の **IDi** をもとに残高を照会するための経路です(モバイルアプリ Mobile Melon 用)。IDi の文字列形が、モバイル Suica / PASMO 等の交通系ICアプリに表示される **カード ID** です。利用者はそれを読み取って入力し、クライアントが IDi(16 桁 hex)へ復元して送ります。相互認証も加盟店認証も伴わない**低信頼・読み取り専用**のパスです。

- 応答は**残高合計と失効内訳のみ**で、生の IDi/IDm・加盟店仮名・その他の識別子は一切返しません。
- IDi は**本文**に載せます(URL には載せない)。これにより IDi がアクセスログに残りません。
- IDi はシステム内で一意なので、`(System Code, IDi)` は 1 口座を指します。
