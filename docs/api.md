...
| 422 | `REFUND_INTO_EXPIRED_BUCKET` | 復元先となるバケットが失効しており復元不可(返金のみ。取消は失効済みでも成立する) | `bucket_id` |
...
### `POST /v1/refunds`  — 返金
本文: `{ "payment_id": "…", "amount": 100 }`(`amount` 省略で残額全部)→ **201**
...
```
過剰返金は **422 `REFUND_EXCEEDS_PAYMENT`**。復元は逆消費順に行われ、**復元先となるバケットが失効していれば 422 `REFUND_INTO_EXPIRED_BUCKET`** でその要求全体を拒否します(部分成立は許しません)。失効前のバケットだけで賄える部分返金は成立します。自店の支払いのみ操作可(他店は 404)。

### `POST /v1/payments/{payment_id}/void`  — 取消
本文なし → **200**、応答は返金と同形(残返金可能額の技術的打消し)。
...
自店の、指定口座(**仮名 `account_id` で指定**、必須)の、返金余地が残る支払い一覧。端末キオスクの返金フローで使用。他店の `account_id` は **404**。バケットの失効は考慮しないため、失効済みバケットの支払いもここに載ります(返金実行時、復元が失効済みバケットに達すると **422 `REFUND_INTO_EXPIRED_BUCKET`**。失効前のバケットだけで賄える部分返金は成立します。取消は失効を理由には拒否されません)。
```json
...
利用者自身が残高を照会するための経路です(モバイルアプリ Mobile Melon 用)。次の 2 通りがあります。いずれも加盟店認証を伴いません。

- **カード ID(手入力)** — 利用者が入力し、クライアントが IDi(16 桁 hex)へ復元して送ります。IDi を**主張するだけ**の低信頼・利便経路です。
- **かざす(相互認証)** — カードを NFC でかざし、サーバ主導の相互認証を中継します。**サーバのみが検証済み IDi を取得**し、応答には残高だけを返します(**IDi はサーバ外に出ません**)。カード実在を証明する高信頼経路です。

いずれも応答は**残高合計と失効内訳のみ**で、生の IDi/IDm・加盟店仮名は一切返しません。識別子は**本文**に載せます(URL には載せない)ので、アクセスログには残りません。

### `POST /v1/self/balance`  — カード ID による残高
本文: `{ "system_code": 3, "idi": "0102030405060708" }`(`system_code` は Suica 系なら `3`)

| ステータス | 条件 |
|---|---|
| **200** | 口座あり。残高(0 も含む)を返す |
| **404** | この `(System Code, IDi)` の口座が存在しない(未登録) |
| **400** | IDi の形式が不正 |
...
### `POST /v1/self/authenticate`  — かざして残高(相互認証)
[`POST /v1/mutual-authentication`](#post-v1mutual-authentication) と**同じ 3 ステップの中継**ですが、加盟店認証は不要です。端末はフレームを中継するだけで、サーバが鍵で認証を駆動します。`XXFEh`(乱数化 IDm)は開始時に **422 `UNSUPPORTED_CARD`** で拒否。無認証のため**レート制限**を掛けます。

- 開始: `{ "idm", "pmm", "system_code", "areas", "services" }` → `{ "step":"auth1", "session_id", "command": { "frame": "…" } }`
- 継続: `{ "session_id", "card_response": "…" }` → 次の `command` … 完了まで
- 完了: `{ "step":"complete", "session_id", "result": { "system_code", "total", "buckets": […] } }`

`result` は**検証済みカードの残高のみ**。IDi(`issue_id`)は返しません。
