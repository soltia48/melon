...
利用者自身が、自分のカードの識別子をもとに残高を照会するための経路です(モバイルアプリ Mobile Melon 用)。識別子は次の 2 通りで、**どちらか一方**を送ります。相互認証も加盟店認証も伴わない**低信頼・読み取り専用**のパスです。

- **IDi**: 文字列形が、モバイル Suica / PASMO 等の交通系ICアプリに表示される **カード ID**。利用者が入力し、クライアントが IDi(16 桁 hex)へ復元して送ります。システム内で一意なので `(System Code, IDi)` は 1 口座を指します。
- **IDm**: カードを **NFC で読み取って**得た製造 ID(16 桁 hex)。同一 `(System Code, IDm)` の生きたバケットを合算します。乱数化・未発行(`XXFEh` 空間)の IDm は拒否します。

応答は**残高合計と失効内訳のみ**で、生の IDi/IDm・加盟店仮名・その他の識別子は一切返しません。識別子は**本文**に載せます(URL には載せない)ので、アクセスログには残りません。

### `POST /v1/self/balance`  — 自分の残高
本文: `{ "system_code": 3, "idi": "0102030405060708" }` または `{ "system_code": 3, "idm": "0102030405060708" }`(`system_code` は Suica 系なら `3`)

| ステータス | 条件 |
|---|---|
| **200** | 口座あり。残高(0 も含む)を返す |
| **404** | この識別子の口座が存在しない(未登録) |
| **422 `UNSUPPORTED_CARD`** | IDm が乱数化・未発行(`XXFEh` 空間) |
| **400** | 形式不正、または `idi`/`idm` を 1 つに絞っていない |
