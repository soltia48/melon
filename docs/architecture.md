...
**Melon 固有の拡張**: セッションは認証時の **system_code** と、完了後の **検証済み IDi** を保持します。

- `authenticated_account(session_id) -> Option<(system_code, idi)>`
- `consume_spend(session_id) -> Result<(system_code, idi)>`: **一回限り**の支払い能力を消費。1 認証 = 1 金銭操作に束縛し、`session_id` のリプレイを防止。
