...
- **返金**:「返金」タブ → カード提示 → その利用者の**返金可能な支払い一覧**から選択 → 金額(既定=返金可能額)→ 実行。照会(要リーダ)と実行(リーダ不要)の 2 段構成。
- 金額入力は**オンスクリーンのテンキー**と**物理キーボード**の両方に対応(数字/Backspace/Escape=クリア)。ただし**支払い/チャージのみ**で、**返金額は一覧から支払いを選んだ後のシート内の入力欄**に直接入力し「返金する」で実行(テンキーは出ません)。Enter は**カード読み取り開始前**の実行ボタン相当(カード待機中〜返金額入力中は効かず、カード待機中は Escape でキャンセル、返金額は「返金する」ボタンで実行、完了表示中は Enter/Escape で閉じる)。
